...
export type Room = { id: string; name: string; original: string; bounds: Rect; finish: 'oak' | 'tile'; area?: number };
...
/** Ground-floor drawing: lift shaft clear construction dimensions 1.67 x 2.41 m. */
export const LIFT: Rect = [7.74,5.05,9.41,7.46];
...
function core(): Wall[] {
  return [
    ...
    wall('stair-right',[7.615,5.05],[7.615,7.85],[],.175),
    wall('lift-back',[7.615,5.05],[9.535,5.05],[],.175),
    wall('lift-right',[9.535,5.05],[9.535,7.46],[],.175),
    wall('lift-front',[7.615,7.46],[9.535,7.46],[],.175),
    // The 1.15 m clear lobby on the drawing is deliberately kept around the shaft.
    wall('lobby-right',[10.78,4.45],[10.78,8], [door(.3,.9)],.24),
  ];
}
export const LEVELS: Level[] = [
  {
    ...
    rooms:[
      ...
      {id:'rear-room',name:'Rear room · full width',original:'Büro 09 / 07 / 03',bounds:[.365,.365,16.565,4.45],finish:'oak'},
      {id:'utility',name:'Utility room',original:'HAR 08',bounds:[3.95,4.65,6.20,7.635],finish:'tile',area:6.93},
      {id:'services',name:'Service room',original:'Elektro / Technik',bounds:[.365,4.65,3.775,7.635],finish:'tile'},
    ...
    walls:[...exterior(0).filter(w=>w.id!=='left'),...core(),
      // Internal door between the garage and the full-width rear room (T30, 1.00 m on the EG sheet).
      wall('garage-connection',[.1825,.1825],[.1825,7.8175],[door(5.62,1.0)],EXTERIOR),
      wall('office-right-front',[10.62,4.03],[16.565,4.03],[door(2.8,1)]),
      wall('service-back',[.365,4.55],[6.32,4.55],[door(2.4,.9),door(4.4,.9)]),
      wall('service-partition',[3.86,4.55],[3.86,7.635]),
      wall('utility-hall',[6.32,3.2],[6.32,4.45],[door(.16,.9)]),
  ...
  },
  {
    ...
    rooms:[
      {id:'studio',name:'Studio',original:'ZBV 18',bounds:[.365,.365,6.20,7.635],finish:'oak',area:34.98},
      {id:'office-16',name:'Main office',original:'Chef 16',bounds:[6.58,-.885,10.35,3.05],finish:'oak',area:14.83},
      {id:'archive',name:'Archive',original:'Archiv 15',bounds:[10.72,.365,12.05,3.05],finish:'oak',area:4.48},
      {id:'accounting',name:'Office',original:'Buchhaltung 14',bounds:[12.23,.365,16.565,3.05],finish:'oak',area:11.70},
      {id:'hall',name:'Hallway',original:'Flur 11',bounds:[4.8,3.23,13.25,4.45],finish:'oak',area:11.31},
      {id:'meeting',name:'Meeting room',original:'Besprechung 13',bounds:[13.45,3.23,16.565,7.635],finish:'oak',area:13.83},
      {id:'wc-west',name:'Bathroom · west',original:'Herren-WC 17',bounds:[4.77,4.65,6.2,7.635],finish:'tile',area:4.85},
    ...
    walls:[...exterior(1),...core(),
      wall('office-left',[6.455,.365],[6.455,3.14],[],.125),
      wall('office-right',[10.475,.365],[10.475,3.14],[],.125),
      wall('archive-right',[12.14,.365],[12.14,3.14],[],.15),
      wall('office-front',[6.455,3.14],[16.565,3.14],[door(.65),door(4.65,.81),door(6.2)],.175),
      wall('studio-hall-back',[4.65,3.14],[6.455,3.14]),
      wall('studio-hall-left',[4.65,3.14],[4.65,7.635],[door(.35,.9)],.125),
  ...
  },
  {
    ...
    rooms:[
      ...
      {id:'bedroom-02',name:'Bedroom 02',original:'Kind 02 · 26',bounds:[10.72,.365,16.565,3.14],finish:'oak',area:15.02},
      {id:'bedroom-main',name:'Main bedroom',original:'Schlafen 25',bounds:[13.165,3.32,16.565,7.635],finish:'oak',area:15.76},
      ...
      {id:'hall',name:'Hallway',original:'Flur 21',bounds:[6.44,3.32,13.015,4.45],finish:'oak',area:9.63},
      {id:'entry',name:'Stair landing',original:'Diele 22',bounds:[6.44,7.95,10.49,8.635],finish:'tile'},
    ],
    walls:[...exterior(2),...core(),
      wall('living-right',[6.32,.365],[6.32,4.45],[door(3.0,.9)],.175),
      wall('bedroom-center-right',[10.53,.365],[10.53,3.23],[],.365),
      wall('bedrooms-front',[6.32,3.23],[16.565,3.23],[door(2.6,.9),door(4.7,.9)],.175),
      wall('bedroom-main-left',[13.09,3.23],[13.09,7.635],[door(.3,.9)],.15),
      wall('bath-back',[10.61,4.55],[13.09,4.55],[door(.3,.9)]),
      wall('laundry-back',[9.35,5.1],[10.61,5.1],[door(.18,.85)]),
    ],
  },
];
...
export const ASSUMPTIONS = [
  ...
  'The lift shaft follows the EG sheet’s 1.67 m by 2.41 m clear dimensions; the surrounding lobby is modeled with a 1.15 m clear side passage.',
